...
const ProductCars = ({ searchTerm }) => {
  ...
  const filteredDesigneWorks = designeWorks.filter((work) =>
    work.hashtag.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className={styles.container}>
      {filteredDesigneWorks.map((designe) => (
        <div key={designe.id} className={styles.card}>
          <div>
            <Link to={`/designe/${designe.id}`}>
              <img
